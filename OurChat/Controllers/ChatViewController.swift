...
//    ]
    //this is an array that holds object
    var messages: [Message] = []
    ...
    func loadMessages() {
        db.collection(K.FStore.collectionName)
            ...
            self.messages = []
            if let e = error {
                print("There was an issue retrieving data from forestore, \(e)")
            }else {
                if let snapshotDocuments = querySnapshot?.documents{
                    for doc in snapshotDocuments{
                        ...
                        if let messageSender = data[K.FStore.senderField] as? String, let messageBody = data[K.FStore.bodyField] as? String{
                            ...
                            //because it is in a closure, and we want to let new data show in a view.
...
extension ChatViewController: UITableViewDataSource {
    ...
    //it should display in each and every row for a UITableViewCell of table view
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //this ia a message from another user
        else{
